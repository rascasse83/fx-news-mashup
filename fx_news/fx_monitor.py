    ...
    for i, sub in enumerate(st.session_state.subscriptions):
        ...
        <div class="currency-card">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h3>{sub['base']}/{sub['quote']}</h3>
                <button id="remove_{key_base}" style="color: #dc3545; background: none; border: none; cursor: pointer;">✕ Remove</button>
            </div>
        """
        ...
        if sub["current_rate"] is not None:
            ...
            # Determine rate direction class
            ...
            if sub["last_rate"] is not None:
                ...
                change_class = "rate-up" if percent_change > 0 else "rate-down" if percent_change < 0 else "rate-neutral"
                sign = "+" if percent_change > 0 else ""
                html += f"""
                <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                    <span>Change:</span>
                    <span class="{change_class}">{sign}{percent_change:.4f}%</span>
                </div>
                """
        else:
            html += '<div>Loading rate data...</div>'
        
        # Add threshold setting
        html += f"""
        <div style="margin-top: 10px;">
            <label style="display: block; margin-bottom: 5px; font-size: 0.8em;">Alert threshold (%)</label>
            <input type="number" id="threshold_{key_base}" value="{sub['threshold']}" min="0.1" step="0.1" style="width: 100%;">
        </div>
        """
        ...
        new_threshold = st.number_input(
            ...
            value=sub["threshold"],
            step=0.1,
            key=f"threshold_input_{key_base}",
...
with col2:
    ...
    if news_items:
        for item in news_items:
            ...
            # Create CSS class based on sentiment
            sentiment_class = f"news-{item['sentiment']}"
            
            # Create HTML for the news card
            html = f"""
            <div class="news-card {sentiment_class}">
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 8px;">
                    ...
                    <div style="display: flex; align-items: center;">
                        ...
                        <span style="
                            background-color: {
                                '#d4edda' if item['sentiment'] == 'positive' else 
                                '#f8d7da' if item['sentiment'] == 'negative' else '#f8f9fa'
                            }; 
                            color: {
                                '#28a745' if item['sentiment'] == 'positive' else 
                                '#dc3545' if item['sentiment'] == 'negative' else '#6c757d'
                            };
                            padding: 2px 6px; 
                            border-radius: 10px; 
                            font-size: 0.8em;">
